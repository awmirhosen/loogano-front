<template>
  <div class="w-full mt-64">
    <div class="">
      <!--Reasons title-->
      <div class="w-full flex justify-start items-center gap-2 gap- px-4 md:my-24">
        <div class="w-full h-1 bg-sky-custom"></div>
        <p class="md:text-[52px] text-[28px] font-morabaa whitespace-nowrap">چرا لوگانو</p>
      </div>

      <div class="w-full mt-8 max-w-[1400px] mx-auto px-10" dir="rtl">

        <!----first----->
        <div class="w-full flex justify-between md:flex-row flex-col">
          <div class="w-full flex order-2 md:order-1">
            <img src="/images/home/first.png" alt="first_section_image">
          </div>
          <div class="w-full md:mt-16 mt-4 order-1">
            <div class="w-full  md:px-10 px-1">
              <p class="md:text-5xl text-2xl text-full text-start font-morabaa">دانش متخصصان</p>
              <p class="text-zinc-600 mt-4 w-full text-start">
                دانش متخصصان ما، اساس راه‌اندازی بهترین سرمایه‌گذاری‌های ملکی را می‌سازد. همراه با تیم حرفه‌ای ما، به سود دست یافته و آینده‌ی مالی خود را به دست بگیرید.
              </p>
            </div>
          </div>
        </div>

        <!----second----->
        <div class="w-full flex justify-between md:mt-10 mt-2 items-center md:flex-row flex-col">

          <div class="w-full mt-16">
            <div class="w-full md:px-10 px-0">
              <p class="md:text-5xl text-2xl text-full text-start font-morabaa">تجربه ممتاز</p>
              <p class="text-zinc-600 mt-4 w-full text-start">
                در دنیای سرمایه‌گذاری ملکی، تجربه ا‌ی ممتاز را با ما داشته باشید. تیم متخصصان ما با دانش عمیق، تحلیل دقیق و ارتباط نزدیک با مشتریان راهکارهای بهترین سرمایه‌گذاری را برای شما فراهم می‌آورد.
              </p>
            </div>
          </div>

          <div class="w-full flex justify-end">
            <img src="/images/home/second.png" alt="first_section_image">
          </div>

        </div>

        <!----third----->
        <div class="w-full flex justify-between mt-10 items-center flex-col md:flex-row">

          <div class="w-full flex justify-start order-2 md:order-1">
            <img src="/images/home/third.svg" class="w-full" alt="first_section_image">
          </div>

          <div class="w-full mt-16 order-1">
            <div class="w-full md:px-10 px-0">
              <p class="md:text-5xl text-2xl text-full text-start font-morabaa">تعهد به موفقیت</p>
              <p class="text-zinc-600 mt-4 w-full text-start">
                موفقیت شما همانند موفقیت ماست. تعهد ما به ارائه نتیجه‌های ارزشمند و سودآور در انتخاب‌های سرمایه‌گذاری، از ویژگی‌های برجسته تیم ماست.
              </p>
            </div>
          </div>

        </div>

      </div>

    </div>
  </div>
</template>

<script setup>

</script>

<style scoped>

</style>