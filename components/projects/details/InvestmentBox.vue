    ...
    <div class="w-full mt-4">
      ...
      <div class="w-full flex justify-between mt-2" dir="rtl">
        ...
        <div class="w-full text-zinc-400 text-left text-[12px]">
          {{ projectEndingDate(projectStore.projectDetails?.project_end) }}
        </div>
      </div>
      ...
      <div class="w-full mt-5" dir="rtl" v-if="projectStore.investmentTimeFlag">
        <div class="w-full flex gap-2 justify-center">
          <div ref="priceBox"
               class="text-center border-b-2 border-[#12788F] pb-2 cursor-pointer transition-all text-[#6EC3D0] w-2/3"
               @click="buyBaseOnPrice">
            بر اساس قیمت
          </div>

          <div ref="meterBox"
               class="border-b-2 w-1/3 text-center border-zinc-200 pb-2 text-zinc-800 cursor-pointer transition-all"
               @click="buyBaseOnMeter">
            بر اساس متر
          </div>
        </div>
        <!----tabs--------->
        <div v-if="invertmentBoxStatus === 1" class="w-full mt-4">
          ...
          <div class="w-full flex justify-center flex-row-reverse mt-4 bg-zinc-100 p-2">
            <div class="border-r-2 border-zinc-200 pr-1" @click.prevent="meterMinus">
              <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 19 20" fill="none">
                <path d="M5.71582 9.91718L13.2676 9.91718" stroke="black" stroke-width="1.13276" stroke-linecap="round"
            ...
            <div class="border-l-2 border-zinc-200 pl-1" @click.prevent="meterPlus">
              <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 19 20" fill="none">
                <path d="M9.78857 6.11426V13.7201M6.0127 9.91719H13.5644" stroke="black" stroke-width="1.13276"
        ...
        <div v-if="invertmentBoxStatus === 2" class="w-full mt-4">
          ...
          <div class="w-full flex justify-center flex-row-reverse mt-4 bg-zinc-100 p-2">
            ...
            <div class="w-full flex items-center">
              <input class="w-full text-center bg-zinc-100" type="number" step="10" placeholder="0"
                     @input="onchangeMeterbox"/>
      ...
      <div class="w-full justify-center flex items-center mt-4 gap-2" v-if="!projectStore.investmentTimeFlag" dir="rtl">
        <div>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path
                d="M10 5.84166V5.83333M10 14.1667L10 8.33333M17.5 10C17.5 14.1421 14.1421 17.5 10 17.5C5.85786 17.5 2.5 14.1421 2.5 10C2.5 5.85786 5.85786 2.5 10 2.5C14.1421 2.5 17.5 5.85786 17.5 10Z"
                stroke="#D4351F" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <p class="text-[12px] text-red-600">فرصت سرمایه گذاری به پایان رسیده</p>
      </div>

      <div class="w-full bg-[#1B9DB1] hover:bg-sky-600 p-3 text-white rounded-lg mt-6 text-center cursor-pointer"
           v-if="projectStore.investmentTimeFlag"
           @click.prevent="projectStore.insertInvest(projectStore.projectDetails.id, totalBoughtMeter)">
        شروع سرمایه گذاری
      </div>
      <div class="w-full bg-zinc-300 p-3 text-white rounded-lg mt-4 text-center cursor-not-allowed" v-else
           disabled="true">
        شروع سرمایه گذاری
      </div>

      <RouterLink to="/profile/wallet/charge"
                  class="w-full block mt-4 hover:text-sky-600 transition-all text-center text-[#1B9DB1]"
                  v-if="projectStore.walletNeed">
        شارژ کیف پول
      </RouterLink>
